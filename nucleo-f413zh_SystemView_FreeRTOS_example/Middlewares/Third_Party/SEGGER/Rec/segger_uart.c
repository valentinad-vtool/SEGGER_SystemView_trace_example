...
void HIF_UART_Init(UART_HandleTypeDef * huart,uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx, UART_ON_RX_FUNC_P cbOnRx);
...
{
	HIF_UART_Init(huart,baud, _cbOnUARTTx, _cbOnUARTRx);
}
...
void HIF_UART_Init(UART_HandleTypeDef * huart,uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx, UART_ON_RX_FUNC_P cbOnRx) {
  ...
  _cbOnRx = cbOnRx;
  _cbOnTx = cbOnTx;
  NVIC_SetPriority(USART2_IRQn, 8);  // Highest prio, so it is not disabled by embOS
